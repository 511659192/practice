...
public class DynamicProxyPerformanceTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        time = System.currentTimeMillis();
        CountService byteBuddyProxy = createByteBuddyDynamicProxy(delegate);
        time = System.currentTimeMillis() - time;
        ...
        for (int i = 0; i < 3; i++) {
            ...
            test(byteBuddyProxy, "run byteBuddy proxy:");
            System.out.println("----------------");
        }
    }
    ...
    public void test() throws Exception {
        ...
        CountService proxy =createByteBuddyDynamicProxy(countService);
        System.out.println(proxy.count());;
    }

    private static CountService createByteBuddyDynamicProxy(CountService delegate) throws Exception {
        DynamicType.Unloaded<CountService> unloadedType = new ByteBuddy()
                .subclass(CountService.class)
                .name(CountService.class.getPackage().getName().concat(".").concat("CountServiceProxy"))
                .method(named("count")).intercept(MethodDelegation.to(ForCount.class))
                .defineField("delegate", CountService.class, Modifier.PRIVATE)
                .implement(DelegateAccess.class)
                .intercept(FieldAccessor.ofBeanProperty())
                .make();
        unloadedType.saveIn(new File("target/classes"));
        Object instance = unloadedType.load(DynamicProxyPerformanceTest.class.getClassLoader())
                .getLoaded()
                .newInstance();
        DelegateAccess access = DelegateAccess.class.cast(instance);
    ...
    static class ForCount {
        ...
        @RuntimeType
        public static Object ByteBuddyInterceptor(
                                                  @FieldValue("delegate") CountService delegate) throws Throwable {
    ...
    static interface DelegateAccess {

        void setDelegate(CountService delegate);
        Object getDelegate();
    }
